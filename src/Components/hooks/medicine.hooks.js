...
export default function useMedicineApis() {
    ...
    const getAllMedicines = async (userId) => {
        try {
            ...
            if (data?.success) {
                ...
                const { medicinesDict, medicineIds } = arrayToReducer(data?.allmedicines)
                dispatch({
                    type: "GET_ALLMEDICINES", payload: {
                        medicinesDict,
                        medicineIds,
                    }
                })
    ...
    const addMedicine = async ({ name, quantity, rate, mrp, hsn, expiry, batchno, mfg, userId }, toast) => {
        try {
